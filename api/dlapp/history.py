...
from .schemas import Designation, Status


def get_history(head, user, db):
    if head and (Role.DEAN in user.roles or Role.HR in user.roles):
        stmt = select(PermissionDB).where(PermissionDB.dean_status != Status.PENDING)
        ...
        stmt = select(PermissionDB).where(
            ...
            and PermissionDB.hod_status != Status.PENDING
        )
        admin_history = db.scalars(stmt).all()
        return admin_history
...
def delete_permission(permission_id, user, db):
    if Role.HR not in user.roles:
        ...
        stmt = delete(PermissionDB).where(PermissionDB.c.permission_id == permission_id)
        db.execute(stmt)
        db.commit()
        return True
